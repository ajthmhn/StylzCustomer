            ...
            if aRes?.statusCode == 200{
                if aRes?.json?["status"].intValue == 200{
                    ...
                    if let data = aRes?.json?["data"].dictionary{
                        ...
                        if let appointments = data["appointments"]?.array{
...
extension AppointmentsViewController: UITableViewDataSource, UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if appointments.count > 0{
            ...
            let formattedDate = getFormatedDate2(dateString:appointments[indexPath.row].start_time ?? "", requiredformat: K.DateFormats.dateLocale)
            let formattedTime = getFormatedDate2(dateString:appointments[indexPath.row].start_time ?? "" , requiredformat: K.DateFormats.timeLocale)
            
            // d/MMM/yyyy h:mm a
            
            
            cell.lblDuration.text = "\(formattedDate) - \(formattedTime)"
            ...
            if appointments[indexPath.row].customer_prepaid == 0{ // need to confrim
        ...
        cell.deleteTappedActions { aCell in
            ...
            //print("lat: \(latDouble), long: \(longDouble)")
            ...
            if (UIApplication.shared.canOpenURL(URL(string:"comgooglemaps://")!)) {  //if phone has an app
                ...
                //print("https://www.google.co.in/maps/dir/?saddr=&daddr=\(latDouble ?? 0.0),\(longDouble ?? 0.0)&directionsmode=driving")
                //Open in browser
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        vc.appointmentId = self.appointments[indexPath.row].id ?? 0
